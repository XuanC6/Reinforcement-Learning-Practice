...
class CNN_Agent(DQN_Agent):
    ...
    def __init__(self, env_name, QNetwork_Class, max_iter_or_epis, dueling = False):
        ...
        self.eps_iter = 50000 # this means epsilon decay from max to min after this number of iterations
        self.max_iter_or_epis = max_iter_or_epis # maximum training episodes
        self.eval_updates = 5000 # interval (number of updates) to do a evaluation when training
        self.copy_updates = 200 # interval (number of updates) to copy the online network to target network
    ...
    def train_replay_memory(self):
        ...
        memory_size = 5000
        self.memory = deque([],maxlen = memory_size)
        burn_in_num = 1000 # number of tuples to burn in memory before training
